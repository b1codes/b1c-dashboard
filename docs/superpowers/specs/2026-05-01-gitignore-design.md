...
We will adopt a "Fail-Secure" approach. Folders containing sensitive logic (like `.agents` and `.claude`) will be ignored entirely by default. We will then explicitly "un-ignore" only the specific files or sub-folders that are safe for public consumption.
...
- **Agent/Tooling:** Protect `.agents/`, `.claude/`, and other future agentic directories using allowlisting.
...
- `.agents/` and `.claude/` ignored by default.
- Explicitly allow:
    - `!.agents/public/`
...
- **Simulated Leak Test:** Attempt to add a dummy secret file (e.g., `.agents/secret_prompt.md`) and verify git ignores it.
